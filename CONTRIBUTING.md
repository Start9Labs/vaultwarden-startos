...
This repo packages [Vaultwarden](https://github.com/dani-garcia/vaultwarden) for StartOS.
...
Vaultwarden runs the upstream `vaultwarden/server` Alpine image. A small helper container is built locally from `argon2.Dockerfile` (used only to Argon2id-hash admin tokens) and tracks `alpine:latest` — it has no pinned version.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `vaultwarden/server:<new version>-alpine`.
